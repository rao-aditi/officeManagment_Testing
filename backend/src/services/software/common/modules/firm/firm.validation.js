...
const dateSchema = z.string().datetime({ offset: true }).or(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)).nullish();
...
const firmSchema = z.object({
  ...
  dateOfDeed: dateSchema,
  residentialStatus: z.enum(["RESIDENT", "NON_RESIDENT", "NOT_ORDINARILY_RESIDENT"]).nullish(),
...
const updateFirmSchema = firmSchema.partial().extend({
  signingPerson: signingPersonSchema.optional(),
});

module.exports = { firmSchema, updateFirmSchema };